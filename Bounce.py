...
class MyGame(arcade.Window):
	...
	def createRetryButton(self,x,y,function):
		button = Button.RetryButton(x,y,function)
		self.addButton(button)
	...
	def on_draw(self):
		...
		if self.current_state == GAME_RUNNING:  self.draw_game()
		if self.current_state == GAME_OVER: self.draw_game_over()
		if self.current_state == WELL_PLAYED: self.draw_well_played()
	...
	def setup(self):
		...
		self.createRetryButton(300,50,self.retryGame)
	...
	def drawScore(self):
		output = f"Score : {self.score}/{len(self.listCibles)}"
		arcade.draw_text(output, 13, 577, [255,255,255,200], 14)
